...
let rightMessage = document.querySelector(".right");
let wrongMessage = document.querySelector(".wrong");
let rightText = document.querySelector(".right-message");
let wrongText = document.querySelector(".wrong-message");

function start() {
  let api = "app/api.json";
...
function changeItem() {
  rightMessage.style.visibility = "hidden";
  check();
  fromLocalStorage();
  addScore();
  ...
  if (e.target.className.includes("active")) {
    ...
    if (mainItem.includes(letter)) {
      ...
      if (right == mainItem.length) {
        rightText.innerHTML = `you got it right its ${mainItem}`;
      ...
      if (mistake == 10) {
        wrongText.innerHTML = `you got it wrong its ${mainItem}`;
